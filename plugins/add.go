...
func addPluginsToManifest(manifest manifestType.Manifest, pluginLocations []string) (manifestType.Manifest, error) {
	...
	if err != nil {
		printer.Error(err, text.ErrorHelpInfo, "")
		return manifest, err
	}

	err = lock.UpdateLock(manifest, lockFile)
	if err != nil {
		printer.Error(err, fmt.Sprintf("%s\n%s", "There was an error updating lockfile. Try your previous command again, and if that fails you may need to remove kombustion.lock entirely.", text.ErrorHelpInfo), "")
		return manifest, err
	}
